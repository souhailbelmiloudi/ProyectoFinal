...
/**
 * Esta clase representa el panel de la ventana "TUS PREGUNTAS".
 */
public class TUS_PREGUNTAS extends JPanel {
	...
	public static String id_jugador;
	
	/**
	 * Crea un nuevo panel de TUS PREGUNTAS.
	 */
	public TUS_PREGUNTAS() {
		...
		add_pre.addMouseListener(new MouseAdapter() {
				...
				try {	       			  
					...
					String consulta2 = "INSERT INTO "+nombretabla()+" (ID_PREGUNTA,PREGUNTAS,id_jugador) VALUES (idPREGUNTAS.NEXTVAL,'"+anadir_Field.getText()+"',"+id_jugador+")";
		...
		borrar_pre.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				conexion.eliminar(nombretabla(), "ID_PREGUNTA ="+borra_Field.getText());
				borra_Field.setText("");	
			}
		});
		...
		//Buscar el ID de las preguntas
		JLabel buscar_pre = new JLabel("¿?");
		buscar_pre.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				conexion.conectar();
				String consulta ="select ID_PREGUNTA , preguntas from "+nombretabla()+"  WHERE preguntas LIKE '%"+ busca_Field.getText()+ "%'";
				preguntas =conexion.consulta(consulta,"ID_PREGUNTA","ID_PREGUNTA", "PREGUNTAs");
	 ...
	 *
	 * @return El nombre de la tabla generado.
	 */
	public String nombretabla() {
		...
		String nombre_tabla = "TABLA_"+Sign_in.usuario.toUpperCase()+"_"+id_jugador;
		return nombre_tabla;
	}	
}
