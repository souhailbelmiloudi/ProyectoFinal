...
/**
 * La clase Mis_preguntas representa una ventana que muestra las preguntas del usuario.
...
public class Mis_preguntas extends JFrame {
	...
	TUS_PREGUNTAS tus = new TUS_PREGUNTAS();

	/**
	 * Constructor de la clase Mis_preguntas. Inicializa la interfaz grafica de la ventana.
	 */
	public Mis_preguntas() {
		...
		btn_hecho.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				p.getNombres();
				String pregunta = conx.consulta("SELECT PREGUNTAS FROM (select PREGUNTAS from " + tus.nombretabla() + " ORDER BY DBMS_RANDOM.VALUE() )  where ROWNUM =1", "PREGUNTAs");
